...
if ($_SERVER['REQUEST_METHOD'] == 'GET') {
  ...
  $errors_ar['gender'] = !empty($_COOKIE['gender_error']);
...
// Иначе, если запрос был методом POST, т.е. нужно проверить данные и сохранить их в XML-файл.
  ...
  if (!isset($gender)) {
    ...
    setcookie('gender_value', $gender, time() + 60 * 60);
    setcookie('gender_error','',100000);
  }
  ...
  if (!isset($limbs)) {
    ...
    setcookie('limbs_value', $limbs, time() + 60 * 60);
    setcookie('limbs_error','',100000);
  }
  ...
  if (!empty($_COOKIE[session_name()]) && !empty($_SESSION['login']) and !$errors) {
    ...
    $upd=$db->prepare("update application set name=:name,mail=:mail,date=:date,gender=:gender,limbs=:limbs,bio=:bio where id=:id");
    $cols=array(
      ':name'=>$fio,
      ':mail'=>$mail,
      ':date'=>$year,
    ...
    if(!$errors){
      ...
      try {
        $stmt = $db->prepare("INSERT INTO application SET name=:name,mail=:mail,date=:date,gender=:gender,limbs=:limbs,bio=:bio");
